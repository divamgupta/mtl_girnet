...
class SharedPrivate_ABSA(Trainer):
    """docstring for SharedPrivate_ABSA"""
    ...
    def cal_cs_psp( self, inp_l , inp_r ):
    
        left_x = self.glove_embed42( inp_l )
        right_x = self.glove_embed42( inp_r )

        left_x_2 = Concatenate(-1)( [ self.rnn_left_2( left_x)   , self.rnn_left_shared( left_x)  ] )
        right_x_2 = Concatenate(-1)( [ self.rnn_right_2( right_x ) , self.rnn_right_shared( right_x ) ] )
        
        left_x_1 = Concatenate(-1)( [ self.rnn_left_( left_x ) , self.rnn_left_shared( left_x )  ] )
        right_x_1 = Concatenate(-1)( [ self.rnn_right_( right_x ) ,  self.rnn_right_shared( right_x ) ] )
    
        return [ left_x_1 , right_x_1 , left_x_2, right_x_2  ]
    
    
    def cal_cs_ssp( self , inp_l , inp_r ):
    
        left_x = self.glove_embed42( inp_l )
        right_x = self.glove_embed42( inp_r )

        left_x_2 = self.rnn_left_2(  Concatenate(-1)( [ ( left_x)   , self.rnn_left_shared( left_x)  ] ))
        right_x_2 = self.rnn_right_2(  Concatenate(-1)( [ ( right_x ) , self.rnn_right_shared( right_x ) ] ))

        left_x_1 = self.rnn_left_(  Concatenate(-1)( [ ( left_x ) ,  self.rnn_left_shared( left_x )  ] ))
        right_x_1 = self.rnn_right_( Concatenate(-1)( [ ( right_x ) ,  self.rnn_right_shared( right_x ) ] ))
    ...
    def cal_cs(self , inp_l , inp_r ):
        ...
        if self.config[ 'sharing_scheme' ] ==  'parallel':
            return self.cal_cs_psp( inp_l , inp_r  )
        elif self.config[ 'sharing_scheme' ] ==  'stacked':
            return self.cal_cs_ssp( inp_l , inp_r  )
...
model = SharedPrivate_ABSA( exp_location="./ttt" , config_args = config )
model.train()
